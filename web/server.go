...
func init() {
	...
	if codes, err := tdx.NewCodesSqlite(tdx.WithCodesClient(client)); err != nil {
		...
		log.Printf("已加载股票代码，共 %d 条", len(codes.GetStocks()))
	}
	...
	manager.AddWorkdayTask("0 30 9 * * *", func(m *tdx.Manage) {
		_ = m
	})
}
...
func handleSearchCode(w http.ResponseWriter, r *http.Request) {
	...
	for _, model := range codeModels {
		fullCode := fmt.Sprintf("%s%s", "sz", model.Code) // protocol.Code doesn't have Exchange
...
func handleCreatePullKlineTask(w http.ResponseWriter, r *http.Request) {
	...
	if len(tables) == 0 {
		...
		for _, v := range tables {
			if v == extend.Day || v == extend.Minute {
	...
	if err != nil {
		errorResponse(w, "创建拉取器失败: "+err.Error())
		return
	}
...
// handleGetIndustries 全市场行业归属（通达信行业 + 申万行业）
...
// handleGetBlocks 板块成分股（概念/地域/行业/指数）
// ?type=gn(概念) | fg(地域) | hy(行业) | zs(指数)，默认概念
func handleGetBlocks(w http.ResponseWriter, r *http.Request) {
	...
	case "hy", "industry":
		file = protocol.BlockFileHY
	case "zs", "index":
		file = protocol.BlockFileZS
	}

	blocks, err := client.GetBlockData(file)
	if err != nil {
		errorResponse(w, err.Error())
		return
	}
